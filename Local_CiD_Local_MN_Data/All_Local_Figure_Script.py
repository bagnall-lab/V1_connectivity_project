...
def Rasterplot_beatnglide(Time, neuron_list,Title):
    ...
    colors1 = ['#572200'.format(i) for i in np.arange(len(neuron_list[0]))]
    colorsf.extend(colors1)
    colors2 = ['#377E21'.format(i) for i in np.arange(len(neuron_list[1]))]
    colorsf.extend(colors2)
    colors3 = ['#000000'.format(i) for i in np.arange(len(neuron_list[2]))]
...
Title = 'Tonic Drive Local CiA_CiD Local CiA_MN 3 Runs'
neuron_list = [VLMN_12,VLCiD_12,VLCiA_12] 
